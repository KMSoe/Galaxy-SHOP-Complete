...
exports.getCart = async (req,res) =>{
  ...
  try {
      ...
      if(cartId[0].id){
        ...
        if(rows.length > 1){
           [totalQuantity,totalBill] = rows.reduce((a,n)=> [a.quantity + n.quantity,((a.price-a.discount)*a.quantity)+((n.price-n.discount)*n.quantity)]);
        }else{
           [totalQuantity,totalBill] = [rows[0].quantity,(rows[0].price-rows[0].discount) * rows[0].quantity];
        }
...
exports.postCart = async (req,res)=>{
  ...
  try {
    ...
    if(rows[0]){
      ...
      cartItem.quantity = 1;
      const [cartItems,feilds] = await CartItem.getCartItemsByProductIdAndCartId(pId,rows[0].id);
      if(cartItems[0]){
...
exports.getCheckout = async (req,res)=>{
  ...
  try { 
    const [cartId,fields] = await Cart.getCartByUserId(userId);
    if(cartId[0].id){
      const [cartItems,fields] = await CartItem.getItemAndProduct(cartId[0].id);
      let totalQuantity,totalBill;
      if(cartItems.length > 1){
        [totalQuantity,totalBill] = cartItems.reduce((a,n)=> [a.quantity + n.quantity,((a.price-a.discount)*a.quantity)+((n.price-n.discount)*n.quantity)]);
      }else{
        [totalQuantity,totalBill] = [cartItems[0].quantity,(cartItems[0].price-cartItems[0].discount) * cartItems[0].quantity];
      }
...
exports.checkout =  async (req,res) =>{
  ...
  try {
    ...
    if(cartId[0].id){
      ...
      const [cartItems,fields] = await CartItem.getItemAndProduct(cartId[0].id);
      if(cartItems.length){
        const order = new Order(userId,req.body.phoneno,req.body.address,0,new Date(),new Date());
...
exports.getOrder = async (req,res)=>{
  ...
  try {
    ...
    for (let i = 0; i < orders.length; i++) {
      const items=[];
      const [orderItems,fields] = await OrderItem.getItemAndProduct(orders[i].id);
  ...
  try {
    ...
    if(products.length){
      ...
      for (let i = 0; i < products.length; i++) {
        const [sellProducts,fields] = await OrderItem.getItemAndProductByProductId(products[i].id);
        const [orders,info] = await Order.getOrderById(sellProducts[i].orderId);
        for (let j = 0; j < orders.length; j++) {
          const item = {};
          item.userId = orders[j].userId;
          item.product = sellProducts[i].name;
          item.quantity = sellProducts[i].quantity;
          item.totalBill = (sellProducts[i].price - sellProducts[i].discount) * item.quantity;
